...
void kalmanFilter(StateVector& currentState, Ricotti& ricotti, const Sensor& sensor,const bool& GPS, const double& magDeclination)
{
    // detect stepsize
    
    //int stepsize{static_cast<int>(millis() - currentState.time)}; // stepsize is in milliseconds
    static bool firstLoop{true};
    static elapsedMicros stepMicros; // step size in micros for accuracy
    uint32_t stepsize{}; // holds step size as read at top of loop in micros
    ...
    currentState.time = millis(); // keep time tracking in millis
    ...
    createZHat_H(zHat, kalmanH, xBar, GPS, magDeclination);
